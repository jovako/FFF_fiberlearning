...
def load_filenames(args: argparse.Namespace) -> list:
    """
    ...
    filenames = json_data["training"]
    filenames = [{
    "image": "ds000001/sub-02/anat/sub-02_T1w.nii.gz",
    "deface_mask": "ds000001/sub-02/anat/sub-02_T1w__Data/deface_mask.nii.gz",
    }]

    data_list = []
    for _i in range(len(filenames)):
        deface_mask_path = filenames[_i]["deface_mask"]
        data_list.append(
            {
                "image": os.path.join(
                    args.data_base_dir, filenames[_i]["image"]
                ),
...
def load_random_mask(
    data_list: list, output_size: tuple, device: torch.device, logger: logging.Logger
) -> torch.Tensor:
    """
    Load a random mask from the MONAI-style data_list and resize to output_size.

    Args:
        data_list (list): List of dicts with keys ["image", "mask", ...].
        output_size (tuple): Desired output size (D, H, W).
        device (torch.device): Torch device.

    Returns:
        torch.Tensor: Mask tensor of shape (1, 1, D, H, W).
    """
    if not data_list:
        raise ValueError("data_list is empty, cannot load mask")

    # Pick random sample
    sample = random.choice(data_list)
    if "mask" not in sample:
        raise KeyError("data_list entry does not contain a 'mask' key")

    mask_file = sample["mask"]
    logger.info(f"Using mask file: {mask_file}")

    latent_size = tuple([d//4 for d in output_size])
    ...
    # scale = ScaleIntensityRange(a_min=0, a_max=1, b_min=-1, b_max=1, clip=True)
    resize = Resize(spatial_size=latent_size, mode="trilinear")
    mask_scaled = mask[None] # scale(mask[None])
    mask_resized = resize(mask_scaled)  # shape (1,1,D,H,W)

    return mask_resized[None].to(device)
...
def run_inference(
    ...
    with torch.amp.autocast("cuda", enabled=True):
        for t, next_t in progress_bar:
            ...
            unet_inputs = {
                "x": torch.cat((image, mask_tensor), dim=1),
        ...
        print(f"Image Mean: {image.mean()}, Image Min: {image.min()}, Image Max: {image.max()}, Image Std: {image.std()}")
...
def diff_model_infer(
    ...
    data_list = load_filenames(args)
    mask_tensor = load_random_mask(data_list, output_size, device, logger)
    data = run_inference(
        args,
        device,
        autoencoder,
