...
class BiomedClipSubjectModel(nn.Module):
    def __init__(self, model_path=f'biomedclip-pretrained-larger-chexpert_{image_size}'):
        super().__init__()
...
if __name__ == "__main__":
    ...
    subject_model_convnext_2 = ConvNextClassfierSubjectModel(f'convnextv2-tiny-chexpert_{image_size}_2').to(device)
    ...
    ndtm_convnext_2 = NDTM(
        generative_model=generative_model,
        subject_model=subject_model_convnext_2,
        hparams=NDTM_config
    )
    ...
    filename = f"sampled_invariances_same_{random_suffix}.pt"
    ...
    for n_batch, batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            test_image_embedding_convnext_2 = subject_model_convnext_2(x)
        originals.append(x)
        original_convnext_embeddings.append(test_image_embedding_convnext)
        original_convnext_2_embeddings.append(test_image_embedding_convnext_2)
        ...
        with torch.no_grad():
            ...
            invariances_convnext_cross_embeddings.append(subject_model_convnext_2(imgs_noised[0].to(device)))    
    
        
        ts = get_timesteps(NDTMTimestepCompatability())
        imgs_noised, imgs_approximated = ndtm_convnext_2.sample(x, None, ts, y_0 = x)
        invariances_convnext_2.append(imgs_noised[0])
        with torch.no_grad():
            invariances_convnext_2_embeddings.append(subject_model_convnext_2(imgs_noised[0].to(device)))
            invariances_convnext_2_cross_embeddings.append(subject_model_convnext(imgs_noised[0].to(device)))

        torch.save({
            "invariances_convnext": torch.cat(invariances_convnext, dim=0),
            "invariances_convnext_2": torch.cat(invariances_convnext_2, dim=0),
            "originals": torch.cat(originals, dim=0),
            "labels": torch.cat(labels, dim=0),
            "invariances_convnext_embeddings": torch.cat(invariances_convnext_embeddings, dim=0),
            "invariances_convnext_2_embeddings": torch.cat(invariances_convnext_2_embeddings, dim=0),
            "original_convnext_embeddings": torch.cat(original_convnext_embeddings, dim=0),
            "original_convnext_2_embeddings": torch.cat(original_convnext_2_embeddings, dim=0),
            "invariances_convnext_cross_embeddings": torch.cat(invariances_convnext_cross_embeddings, dim=0),
            "invariances_convnext_2_cross_embeddings": torch.cat(invariances_convnext_2_cross_embeddings, dim=0),
        }, filename)
